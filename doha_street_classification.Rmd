...
```{r plot diagnostics, fig.cap="Diagonstics of model training"}
# we saved the diagnostics to a png file to save computation time
include_graphics("images/diagnostics.png")
# <p style="text-align: center;">![Diagonstics of model training](images/diagnostics.png)</p>
```
...
```{r fig.cap= "Prediciton sample, left image is ground truth, image in the middle is the input subset of Bahrain, image on the right is the prediction"}
if(!file.exists("pretrained_unet.h5")){
...
```{r prediction sample, fig.cap="Prediciton sample, ground truth (left), input subset of Bahrain (center), prediction (right)"}
# again saved 
# <p style="text-align: center;">![Prediciton sample, left image is ground truth, image in the middle is the input subset of Bahrain, image on the right is the prediction](images/prediction_sample.png)</p>
include_graphics("images/prediction_sample.png")
```
...
```{r sample 25 2017, fig.cap="Sample image no 25. (2017)", message=F}
par(mfrow=c(1,1))
...
In the Figure 'Classified Street Pixels comparison' it can be seen that street pixels have more than doubled from 2021 - 2017 and are stable between 2017 and 2019. This leads to the assumption that there must be a construction boom between 2019 and 2021. This assumption is solidified when analyzing the images below.

```{r,echo=F,warning=F, message=F,results=T,out.width="100%"}
mapviewOptions(basemaps = c("Esri_WorldImagery", "OpenStreetMap.DE"),
...
viewRGB(stack("data/testarea_unet/doha_2017.tif"),layer.name = "Doha 2017 (RGB)", 
  ...
  viewRGB(stack("data/testarea_unet/doha_2021.tif"),layer.name = "Doha 2021 (RGB)", 
          quantiles = c(0,1),r=1,g=2,b=3)+
  mapview(result_map_17,layer.name="street prediction in Doha 2017", alpha.regions=1,na.alpha=0)+
  mapview(result_map_19,layer.name="street prediction in Doha 2019", alpha.regions=1,na.alpha=0)+
  mapview(result_map_21,layer.name="street prediction in Doha 2021", alpha.regions=1,na.alpha=0)
```

```{r plot prediction results, fig.height=12, fig.cap="Street pixel probabilities (left), satellite image (right)", echo=FALSE}
par(mfrow=c(3,2))
...
```{r plot prediction results development, fig.height=12, fig.cap="Development", echo=FALSE}
col_pal <- colorRampPalette(c("red","white","green"))
par(mfrow=c(3,1))
plot(result_map_21- result_map_17, main = "2017 to 2021", col=col_pal(3))
plot(result_map_21- result_map_19, main = "2019 to 2021", col=col_pal(3))
plot(result_map_19- result_map_17, main = "2017 to 2019", col=col_pal(3))
```

```{r plot binary prediction results, fig.height=12, fig.cap="Binary classifaction (left), development (right)"}
...
```{r view rgb of doha from 2017 to 2019, fig.cap="Doha 2017 (top), Doha 2021 (bottom)"}
...
The satellite images from 2017 (Figure 4) and 2021 (Figure 5) show that the horizontal main road in the north of the Khalifa stadium was under construction in 2017 and finished in 2021. This is visible in our 2017-2021 plot. 

The removed streets on the top right of Figure 4 are displayed in our prediction as well. The road was extended to a highway, so roundabouts and junctions were removed.
